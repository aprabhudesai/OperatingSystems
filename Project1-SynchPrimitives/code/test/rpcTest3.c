...
main()
{
	...
	WriteToConsole("\nAcquire lock1: %d in rpcClient3\n\0", lock1, -1, -1);
	if((Acquire(lock1)) == -1){
		WriteToConsole("Error acquiring lock1 with id:%d in rpcClient3\n\0",lock1,-1,-1);
		Exit(0);
	}
	
	if((Signal(cv1, lock1)) == -1){
			WriteToConsole("Error signalling on cv1 and lock1 in rpcClient1\n\0",-1,-1,-1);
			Exit(0);
	}
