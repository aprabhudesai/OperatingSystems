...
main()
{
	int lockId, lockStatus, cvId, cvStatus, mvId, mvStatus, mvValue, waitStatus;
	...
	if(waitStatus == -1)
	{
		WriteToConsole("\nFailed to Wait on the given Condition\0", -1, -1, -1);
		Exit(0);
	}
	else
	{
	...
	{
		WriteToConsole("\nGot value from MV at index 2 : %d.\n\0", mvValue, -1, -1);
	}
