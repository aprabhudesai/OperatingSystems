...
int main(){
	...
	WriteToConsole("Lock1 created in rpcClient2\n\0",-1,-1,-1);
	WriteToConsole("cv1 created in rpcClient2\n\0",-1,-1,-1);
	WriteToConsole("mv1 created in rpcClient2\n\0",-1,-1,-1);

	WriteToConsole("\nAcquiring lock1 in rpcClient2\n\0", -1, -1, -1);
	if((Acquire("lock1",5)) == -1){
		WriteToConsole("Error acquiring lock1 in rpcClient2\n\0",-1,-1,-1);
		Exit(0);
	}
